...
import { persistentMenuKeyboard, mainMenuKeyboard } from "../keyboards/index.js";
...
startHandler.command("start", async (ctx) => {
  ...
  if (existing) {
    await ctx.reply(
      ...
      {
        parse_mode: "HTML",
        reply_markup: persistentMenuKeyboard(existing.role),
      }
    );
    return;
  }
  ...
  if (telegramId === config.ownerTelegramId) {
    ...
    await ctx.reply(
      ...
      { reply_markup: persistentMenuKeyboard("OWNER") }
    );
    return;
  }
  ...
  try {
    ...
    await ctx.reply(
      ...
      { reply_markup: persistentMenuKeyboard(invite.role) }
    );
  } catch (err: any) {
...
startHandler.on("message:text", async (ctx, next) => {
  ...
  await ctx.reply(
    ...
    {
      parse_mode: "HTML",
      reply_markup: persistentMenuKeyboard(user.role),
    }
  );
});

// ─── Main menu callback (inline) ──────────────────────────────────────────────
