...
export function persistentMenuKeyboard(role: Role): Keyboard {
  ...
  if (isCreator) {
    kb.text("➕ Создать задачу").text("📋 Мои задачи").row();
  }

  if (isExecutor) {
    kb.text("📋 Очередь задач").row();
    kb.text("📌 Мои задачи").text("✅ Завершённые").row();
    kb.text("📊 Моя статистика").text("🏆 Рейтинг").row();
  }
  ...
  if (isManager) {
    ...
    kb.row();
    kb.text("👥 Команды").text("👤 Сотрудники").row();
  }

  if (["OWNER", "HEAD", "TEAM_LEAD"].includes(role)) {
    kb.text("🔗 Создать инвайт").row();
  }

  return kb.resized().persistent();
}

// ─── Inline Main Menu (for inline callback messages) ──────────────────────────
...
export function mainMenuKeyboard(role: Role) {
  ...
  if (isManager) {
    kb.text("📊 Статистика", "stats:view");
    if (role === "OWNER" || role === "HEAD") {
      kb.text("📋 Все задачи", "all:tasks");
    }
    kb.row();
    kb.text("👥 Команды", "teams:list").text("👤 Сотрудники", "staff:list").row();
  }
...
export function staffListKeyboard(
  users: { id: number; firstName: string; role: Role; isActive: boolean }[]
) {
  const kb = new InlineKeyboard();
  users.forEach((u) => {
    const status = u.isActive ? "" : " 🚫";
    kb.text(`${ROLE_LABELS[u.role]} ${u.firstName}${status}`, `staff:view:${u.id}`).row();
  });
  kb.text("🔙 Назад", "menu:main");
  return kb;
}

export function staffActionKeyboard(userId: number, isActive: boolean) {
  const kb = new InlineKeyboard();
  if (isActive) {
    kb.text("🚫 Деактивировать", `staff:deactivate:${userId}`).row();
  } else {
    kb.text("✅ Активировать", `staff:activate:${userId}`).row();
  }
  kb.text("🔙 К списку", "staff:list");
  return kb;
}
